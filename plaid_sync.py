...
def _ensure_visibility_tables(conn: sqlite3.Connection) -> None:
    ...
    # Safely alter transactions table if needed
    tx_columns = [
    ...
    for column, dtype in tx_columns:
...
def resets_cursors(conn: sqlite3.Connection, user_id: Optional[str] = None) -> None:
    """Reset Plaid sync cursors for a user or globally."""
    _ensure_cursor_table(conn)
    c = conn.cursor()
    if user_id:
        c.execute("DELETE FROM plaid_cursors WHERE user_id = ?", (str(user_id),))
    else:
        c.execute("DELETE FROM plaid_cursors")
    conn.commit()


# Alias for backward compatibility
reset_cursors = resets_cursors
...
async def _handle_new_transaction(
    conn: sqlite3.Connection,
    ...
    *,
    ...
    if pending_transaction_id:
        c.execute("DELETE FROM transactions WHERE transaction_id = ?", (pending_transaction_id,))
    c.execute(
        """
...
def _handle_removed_transaction(conn: sqlite3.Connection, removed: dict) -> None:
            ...
            while has_more:
                ...
                try:
                    ...
                    for txn in data.get("modified", []):
                        await _handle_modified_transaction(conn, tx_queue, txn, user_id=user_id)
                    for r in data.get("removed", []):
                        _handle_removed_transaction(conn, r)
...
async def _send_transaction_alert(
    ...
    await channel.send(
        ...
        f" Account Used: {account_name}\n"
        f" Total Liquid Cash: ${total_liquid:.2f}\n"
        f" Net Cash: {_format_net_cash(net_cash)}\n"
        f" Date & Time: {txn_date}\n"
        f" Transaction ID: `{transaction_id}`"
    )


async def run_plaid_accounting_pass(
    conn: sqlite3.Connection,
    tx_queue: asyncio.Queue,
    bot=None,
    channel_id: Optional[int] = None,
...
def discover_polling_targets(
    base_dir: str = "data",
    default_db: str = "data/finances.db",
    default_user_id: Optional[str] = None,
) -> List[Tuple[str, str]]:
    """
    Discover all (db_path, user_id) pairs that should be checked by the Plaid sync engine.
    Finds:
    1. The primary database ('data/finances.db') and all users who have Plaid credentials or records.
    2. Any partitioned user databases ('data/finances_<uid>.db').
    """
    targets = []
    seen = set()
    ...
    if os.path.exists(default_db):
        try:
            ...
            user_ids = set()
            try:
                cur.execute("SELECT DISTINCT user_id FROM user_info WHERE user_id IS NOT NULL AND user_id != ''")
                user_ids.update(r[0] for r in cur.fetchall())
            except sqlite3.OperationalError:
                pass

            try:
                cur.execute("SELECT DISTINCT user_id FROM plaid_accounts WHERE user_id IS NOT NULL AND user_id != ''")
                user_ids.update(r[0] for r in cur.fetchall())
            except sqlite3.OperationalError:
                pass

            try:
                cur.execute("SELECT DISTINCT user_id FROM transactions WHERE user_id IS NOT NULL AND user_id != '' LIMIT 10")
                user_ids.update(r[0] for r in cur.fetchall())
            except sqlite3.OperationalError:
                pass
            ...
            if not user_ids:
                uid = default_user_id or os.getenv("CURRENT_USER_ID") or os.getenv("DEFAULT_USER_ID") or "default_user"
                user_ids.add(uid)

            for uid in sorted(user_ids):
                pair = (default_db, str(uid))
                if pair not in seen:
                    seen.add(pair)
                    targets.append(pair)
        except Exception as exc:
            logger.warning(f"Could not inspect primary DB {default_db}: {exc}")
            uid = default_user_id or "default_user"
            pair = (default_db, uid)
            if pair not in seen:
                seen.add(pair)
                targets.append(pair)

    # 2. Partitioned multi-user databases
    pattern = os.path.join(base_dir, "finances_*.db")
    for db_path in sorted(glob.glob(pattern)):
        filename = os.path.basename(db_path)
        uid = filename.replace("finances_", "").replace(".db", "")
        if uid:
            pair = (db_path, uid)
            if pair not in seen:
                seen.add(pair)
...
async def plaid_polling_loop(conn: sqlite3.Connection, tx_queue: asyncio.Queue, bot, channel_id: int):
    """
    ...
    while True:
        try:
            targets = discover_polling_targets()
            for db_path, uid in targets:
