...
def validate_url(url: str) -> tuple[bool, str]:
    ...
    url_pattern = re.compile(
        ...
        r'localhost|'  # localhost (will be blocked by SSRF check)
...
def sanitize_path(user_path: str, base_directory: str) -> str:
    ...
    if not base_directory or not isinstance(base_directory, str):
        raise PathTraversalError("Base directory must be specified")
    
    # Convert to absolute paths
    base_abs = Path(base_directory).resolve()
    
    # Handle user path - join with base if relative
    if os.path.isabs(user_path):
        user_abs = Path(user_path)
    else:
        user_abs = base_abs / user_path
    
    # Resolve to absolute path (resolves .., symlinks, etc.)
    try:
        user_resolved = user_abs.resolve(strict=False)
    except (OSError, ValueError) as e:
        raise PathTraversalError(f"Invalid path: {e}")
    
    # Check containment using commonpath
    try:
        common = os.path.commonpath([str(base_abs), str(user_resolved)])
        if Path(common) != base_abs:
            raise PathTraversalError(
                f"Path traversal detected: {user_path} resolves outside {base_directory}"
            )
    except ValueError:
