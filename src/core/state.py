...
# Optional classifier-specific model.  It defaults to the existing advisor
# model so deployments that do not configure a separate classifier retain the
# exact current behavior; the advisor model setting itself is untouched.
CLASSIFIER_MODEL = os.getenv("CLASSIFIER_MODEL", ADVISOR_MODEL)
...
def get_cloud_temperature(*, tool_mode: bool = False) -> float:
    """Return tuned sampling temperature for cloud models (default 0.7)."""
    raw = (
        (os.getenv("CLOUD_TOOL_TEMPERATURE") if tool_mode else None)
        or (os.getenv("TOOL_TEMPERATURE") if tool_mode else None)
        or os.getenv("CLOUD_CHAT_TEMPERATURE")
        or os.getenv("CHAT_TEMPERATURE_CLOUD")
        or os.getenv("CLOUD_TEMPERATURE")
        or os.getenv("CHAT_TEMPERATURE", "0.7")
    )
    try:
        return float(raw)
...
def get_local_temperature(*, tool_mode: bool = False) -> float:
    """Return tuned sampling temperature for local models (default 0.6)."""
    raw = (
        (os.getenv("LOCAL_TOOL_TEMPERATURE") if tool_mode else None)
        or (os.getenv("TOOL_TEMPERATURE") if tool_mode else None)
        or os.getenv("LOCAL_CHAT_TEMPERATURE")
        or os.getenv("CHAT_TEMPERATURE_LOCAL")
        or os.getenv("LOCAL_TEMPERATURE")
        or os.getenv("CHAT_TEMPERATURE", "0.6")
    )
    try:
        return float(raw)
    except (ValueError, TypeError):
...
CLOUD_CHAT_TEMPERATURE = get_cloud_temperature()
LOCAL_CHAT_TEMPERATURE = get_local_temperature()
