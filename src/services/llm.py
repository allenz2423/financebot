...
- Schedule the first iteration with schedule_reminder.
- CRITICAL: In the reminder's instruction, tell your future self to schedule the next iteration, passing the same instruction forward to keep the loop alive.
- For FINITE loops (e.g., "run this 4 times"), pass countdown state forward: "This is iteration 2 of 4... Stop scheduling when you reach 4."
- You are a programmable agent: use the instruction payload to pass state, context, and future instructions to yourself. Never tell the user a finite loop is impossible.
...
"""
    ...
    audit_batch_mode = bool(
        prior_audit.get("active")
        or explicit_audit_request
        or inherited_audit_intent
    ...
    while True:
        ...
        total_calls_so_far = sum(tool_call_counts.values())
        ...
        if tool_calls:
            ...
            # tool schema.
            if _audit_is_active():
                audit_getter_names = {"get_unlocked_transactions", "get_locked_transactions", "get_unique_unregistered_merchants"}
                getter_calls = [
            ...
            if _audit_is_active():
                ...
                if research_inflight:
                    ...
                    if any(
                        name not in {"search_web", "fetch_webpage", "crawl_deeper"}
            ...
            for tool_call in tool_batch:
                ...
                try:
                    ...
                    if func_name == "query_spending":
                        ...
                        c.execute("CREATE TABLE IF NOT EXISTS scheduled_reminders (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, trigger_at TEXT, instruction TEXT, status TEXT DEFAULT 'pending', channel_id INTEGER)")
                        try:
                            c.execute("ALTER TABLE scheduled_reminders ADD COLUMN channel_id INTEGER")
                        except Exception:
                            pass
                        trigger_val = str(args.get("trigger_time", "")).strip()
                        ...
                        channel_id_val = None
                        if hasattr(reply_msg, "channel") and hasattr(reply_msg.channel, "id"):
                            channel_id_val = reply_msg.channel.id
                            
                        c.execute("INSERT INTO scheduled_reminders (user_id, trigger_at, instruction, channel_id) VALUES (?, ?, ?, ?)", (uid, trigger_val, args.get("instruction"), channel_id_val))
                        conn.commit()  # Force write immediately so the watchdog sees it!
                        db_result = f" Reminder scheduled for {trigger_val}."
