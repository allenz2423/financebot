...
Proposed Sol: Provide a Muse-like persistent, owner-scoped computer experience without requiring a heavyweight always-on hypervisor VM: a durable workspace/tools/browser profile as explicitly configured by the owner, plus isolated disposable task execution and scratch overlays. Start with an opt-in, no-network worker/container with configurable CPU, memory, process, output, and runtime limits; transfer files through explicit bounded operations. Add interactive browser control only behind a per-task browser profile/broker and SSRF-resistant egress policy; credentials stay executor-only, user takeover pauses automation, and financial authorization/receipts remain in the host runtime. A microVM is an optional stronger backend, not a requirement for self-hosting. The owner chose one bot process per task database; multi-process and multi-host leases remain disabled. The bot remains the sole task-DB owner; the worker manager must not open or mount that DB. Remote-model access to computer content must follow explicit provider/data-sharing configuration. See [Muse secure-environment research and phased design](muse-secure-environment-research.md) for the published Muse architecture comparison and Delilah-specific worker constraints.
